...
namespace SvgComposition.ElementControls.GraphicElements
{
    /// <summary>
    /// Interaction logic for SvgLine.xaml
    /// </summary>
    public partial class SvgImage : UserControl, ISvgElementControl, INotifyPropertyChanged
    {
        ...
        {
            "id", "preserveAspectRatio", "class", "style", "externalResourcesRequired", "transform"
        };

        private List<string> _startWith = new List<string>
        {
            "x", "y", "height", "width", "href", 
        };
...
#region configure 
     ...
     public void BuildPermittedContent()
        {
            foreach (string st in SvgElementCreator.DescriptiveElements)
            {
                if (_permitedChildElements.Contains(st) == false)
                {
                    _permitedChildElements.Add(st);
                }
            }

            foreach (string st in SvgElementCreator.AnimationElements)
            {
                if (_permitedChildElements.Contains(st) == false)
                {
                    _permitedChildElements.Add(st);
