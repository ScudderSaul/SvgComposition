...
{
    /// <summary>
    /// Interaction logic for SvgfeDiffuseLighting.xaml
    /// </summary>
    public partial class SvgfeDiffuseLighting : UserControl, ISvgElementControl, INotifyPropertyChanged
    {
        ...
        {
            "feDistantLight", "fePointLight", "feSpotLight"   // one only
        };
        ...
        {
            "id", "class", "style", "result", "surfaceScale", "diffuseConstant", "kernelUnitLength",
        };
        ...
        public SvgfeDiffuseLighting()
        {
            this.Visibility = Visibility.Visible;
            this.BorderBrush = new SolidColorBrush(Colors.Black);
            this.BorderThickness = new Thickness(1.0);
            BuildPermittedContent();
            SvgElementTypeName = "feDiffuseLighting";
        ...
        private void OnlyOneLightSource()
        {
            var qq = from cc in _svgElement.SvgElements
                select cc.ElementType;
            ...
            if (qq.Any())
            {
                ...
                {
                    if (SvgElementCreator.LightSourceElements.Contains(ss))
                    {
                        onein = true;
                    }
                }
            ...
            if (onein == true)
            {
                foreach (string ee in SvgElementCreator.LightSourceElements)
                {
                    _permitedChildElements.Remove(ee);
                }
            }
            else
            {
                foreach (string ee in SvgElementCreator.LightSourceElements)
                {
                    if (_permitedChildElements.Contains(ee) == false)
                    {
                        _permitedChildElements.Add(ee);
                    }
                }
            }
        }
