...
namespace SvgComposition.ElementControls.AnimationElements
{
    /// <summary>
    /// Interaction logic for SvgAnimateMotion.xaml
    /// </summary>
    public partial class SvgAnimateMotion : UserControl, ISvgElementControl, INotifyPropertyChanged
    {
        ...
        };  // "fill" is different for animate

        private List<string> _startWith = new List<string>
        {
           "calcMode", "path", "keyPoints", "rotate", 
        ...
        public SvgAnimateMotion()
        {
            ...
            SvgElementTypeName = "animateMotion";
            InitializeComponent();
            DataContext = this;
        }
        ...
        public void BuildUnusedAttributeList()
        {
            ...
            foreach (string st in SvgAttributeCreator.AnimationTimingAttributes)
            {
                if (_attributeChoices.Contains(st) == false)
                {
                    _attributeChoices.Add(st);
            ...
            foreach (string st in SvgAttributeCreator.AnimationValueAttributes)
            {
                if (_attributeChoices.Contains(st) == false)
                {
                    _attributeChoices.Add(st);
            ...
            foreach (string st in SvgAttributeCreator.AnimationAdditionAttributes)
            {
                if (_attributeChoices.Contains(st) == false)
                {
                    _attributeChoices.Add(st);
            ...
            foreach (string st in SvgAttributeCreator.AnimationEventAttributes)
            {
                if (_attributeChoices.Contains(st) == false)
                {
                    _attributeChoices.Add(st);
